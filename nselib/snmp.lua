...
Helper = {
  --- Name of credentials storage holding discovered communities
  communities_store = "snmp.communities",
  ...
  new = function( self, host, port, community, options )
    ...
    if community then o.community = community
    else
      local creds_store = creds.Credentials:new(o.communities_store, host, port)
      local account = creds_store:getCredentials(creds.State.VALID)()
      if (account and account.pass) then
        if account.pass == "<empty>" then o.community = ""
        else o.community = account.pass end
      else o.community = "public" end
    end
