...
local sniff_snmp_responses = function(host, port, lport, result)
  ...
  local communities = creds.Credentials:new(snmp.Helper.communities_store, host, port)
...
action = function(host, port)
  ...
  if result.status then
    return creds.Credentials:new(snmp.Helper.communities_store, host, port):getTable()
  else
